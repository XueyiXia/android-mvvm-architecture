...
abstract class PortfolioVH(itemView: View) : RecyclerView.ViewHolder(itemView) {
  ...
  protected val positiveColor: Int = ContextCompat.getColor(itemView.context, com.google.android.material.R.color.design_default_color_primary)
  protected val negativeColor: Int = ContextCompat.getColor(itemView.context, com.framework.http.R.color.color_666666)
  protected val neutralColor: Int by lazy {
      try {
          val typedValue = TypedValue()
          val theme: Theme = itemView.context.theme
          theme.resolveAttribute(
              R.attr.disable_slider_refresh,
              typedValue,
              true
          )
          ContextCompat.getColor(itemView.context, typedValue.data)
      } catch (_: Exception) {
          ContextCompat.getColor(itemView.context, com.google.android.material.R.color.design_default_color_primary)
      }
  }
  ...
  @Throws(Exception::class) fun update(
    quote: Quote?,
    listener: StocksAdapter.QuoteClickListener?=null
  ) {
    if (quote == null) {
      return
    }
